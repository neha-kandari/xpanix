...
const SignupForm: React.FC = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Firestore check failed, but user is authenticated:", firestoreError);
        // User is authenticated, just show phone modal
        setShowPhoneModal(true);
      }
    } catch (err: unknown) {
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Saving phone number for user:", auth.currentUser?.uid);
